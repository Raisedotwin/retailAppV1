...
const AffiliatePage: React.FC = () => {
  ...
  const referrer = searchParams.get('affiliate'); // Capture affiliate from URL

  useEffect(() => {
    // Generate the affiliate ID based on user wallet or ID
    //const wallet = getEmbeddedConnectedWallet(wallets);
    const wallet = user?.wallet?.address;
    if (wallet) {
      const affiliateId = wallet; // Use wallet address as affiliate ID
      setAffiliateId(affiliateId);
      // Set affiliate link for sharing
      setAffiliateLink(`${window.location.origin}/wallet?affiliate=${affiliateId}`);
    }
  }, [wallets]);

  // Optionally store affiliate ID for future transactions
  useEffect(() => {
    if (referrer) {
